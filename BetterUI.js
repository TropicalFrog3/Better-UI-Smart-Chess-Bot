// ==UserScript==
// @name        Better UI Smart Chess Bot
// @name:fr     Better UI Smart Chess bot
// @namespace   TropicalFrog3, sayfpack13
// @author      TropicalFrog3, sayfpack13
// @version     2.0
...
var enableUserLog = false;// enable interface log
var enableEngineLog = false;// enable engine log
...
function toggleGUI() {
    ...
    if (iframes.length > 0) {
        const guiFrame = iframes[0];
        if (guiVisible) {
            guiFrame.style.visibility = 'hidden';
            lastGuiWidth = guiFrame.style.width;
            guiFrame.style.width = '0px';
            lastGuiHeight = guiFrame.style.height ;
            guiFrame.style.height = '0px';
            guiVisible = false;
        } else {
            guiFrame.style.visibility = 'visible';
            guiFrame.style.width = lastGuiWidth;
            guiFrame.style.height = lastGuiHeight;
            guiVisible = true;
        }
    } else {
...
function moveResult(from, to, power, clear = true) {
    ...
    for (let a = 0; a < possible_moves.length && engineIndex != 0; a++) {
...
Gui.settings.window.title = 'Better UI Smart Chess Bot - Press [G] to open / close';
...
function InterfaceUtils() {
    this.boardUtils = {
        ...
        markMove: (fromSquare, toSquare, rgba_color) => {
            ...
            if (CURRENT_SITE == CHESS_COM && fromSquare != "" && toSquare != "") {
 
                if (!isNotCompatibleBrowser()) {
                    let [fromElem, toElem] = [this.boardUtils.findSquareElem(fromSquare), this.boardUtils.findSquareElem(toSquare)];
                    if(fromElem != null && toElem != null)
                    {
                        fromElem.style.scale = 1;
                        toElem.style.scale = 1;
...
function markMoveToSite(fromSquare, toSquare) {
    const highlight = (fromSquare, toSquare) => {
        const arrowId = `arrow-${fromSquare}${toSquare}`;
        const arrowData = `${fromSquare}${toSquare}`;
 
        const points = calculateArrowPoints(fromSquare, toSquare);
        const pointsString = points.map(point => `${point.x} ${point.y},`).join('     ');
 
        const rotArrow = CalculateArrowRotation(fromSquare, toSquare);
 
        const arrowElem = document.createElementNS("http://www.w3.org/2000/svg", "polygon");
        arrowElem.setAttribute("id", arrowId);
        arrowElem.setAttribute("data-arrow", arrowData);
        arrowElem.setAttribute("class", "arrow");
        arrowElem.setAttribute("points", pointsString);
        arrowElem.setAttribute("transform",`rotate(${rotArrow.deg} ${rotArrow.param1} ${rotArrow.param2})`);
        arrowElem.setAttribute("style", `fill: rgba(255, 170, 0, 0.8); opacity: 0.8;`);
 
        activeSiteMoveHighlights.push(arrowElem);
        ...
        let arrowsSVG = chessBoardElem.querySelector('svg.arrows');
        if (!arrowsSVG) {
            arrowsSVG = document.createElementNS("http://www.w3.org/2000/svg", "svg");
            arrowsSVG.setAttribute("class", "arrows");
            chessBoardElem.appendChild(arrowsSVG);
        }
 
        arrowsSVG.appendChild(arrowElem);
    }
 
    highlight(fromSquare, toSquare);
}
 
function CalculateArrowRotation(fromSquare, toSquare) {
    const startAngle = -90;
 
    const fromCoords = squareToCoords(fromSquare, true);
    const toCoords = squareToCoords(toSquare, true);
    const deltaX = toCoords.x - fromCoords.x;
    const deltaY = toCoords.y - fromCoords.y;
    const angleRad = Math.atan2(deltaY, deltaX);
    const angleDeg = angleRad * (180 / Math.PI);
 
    const rotationAngle = angleDeg + startAngle;
 
    const centerX = fromCoords.x;
    const centerY = fromCoords.y;
 
    return { deg: rotationAngle, param1: centerX, param2: centerY };
}
 
 
function calculateArrowPoints(fromSquare, toSquare) {
 
    const fromCoords = squareToCoords(fromSquare, true);
    const toCoords = squareToCoords(toSquare, false);
 
    const lengthX = Math.abs(fromCoords.x - toCoords.x);
    const lengthY = Math.abs(fromCoords.y - toCoords.y);
    const lengthArrow = Math.sqrt(Math.pow(lengthX, 2) + Math.pow(lengthY, 2));
 
    const arrowPoints = [];
 
    arrowPoints.push({ x: fromCoords.x - 1.375, y: fromCoords.y + 4.5}); // top left
    arrowPoints.push({ x: fromCoords.x - 1.375, y: fromCoords.y + lengthArrow - 4.25 }); // bottom left
    arrowPoints.push({ x: fromCoords.x - 3.25, y: fromCoords.y + lengthArrow - 4.25 }); // bottom left left
 
    arrowPoints.push({ x: fromCoords.x, y: fromCoords.y + lengthArrow }); // bottom
 
    arrowPoints.push({ x: fromCoords.x + 3.25, y: fromCoords.y + lengthArrow - 4.25}); // bottom right right
    arrowPoints.push({ x: fromCoords.x - 1.375 + 2.75, y: fromCoords.y + lengthArrow - 4.25}); // bottom right
    arrowPoints.push({ x: fromCoords.x - 1.375 + 2.75, y: fromCoords.y + 4.5 }); // top right
    return arrowPoints;
}
 
function squareToCoords(square, start) {
    var xLettre = square.charAt(0);
    var yLettre = square.charAt(1);
    var y = 0;
    var x = 0;
...
function addGuiPages() {
    ...
    <div class="rendered-form" id="main-tab">
                ...
                .row-5277c .square-55d63{
                display:flex;
                min-width: 25px;
                min-height: 25px;
                }
 
                .row-5277c .square-55d63 .notation-322f9{
                display:none;
                }
...
#board .chessboard-63f37 .board-b72b1{
min-height:200px;
min-width:200px;
}
...
transition: 0.2s;
        ...
        <div class="card">
            <div class="card-body" id="chessboard" style="display:none">
                <div class="main-title-bar" style="transition: 0.2s;">
            ...
            <div id="orientation" style="display:none;" class="hidden"></div>
            <div class="card-footer sideways-card"><input class="btn" type="button" value="View Chessboard" id="viewchess-btn"></input></div>
            <div class="card-footer sideways-card"><input class="btn" type="button" value="Get Best Move" id="bestmove-btn"></input></div>
            <div class="card-footer sideways-card" style="visibility:hidden;width:0px;height:0px;padding: 0 0 0 0;">FEN :<div style="visibility:hidden;width:0px;height:0px;padding: 0 0 0 0;" id="fen"></div></div>
            <div class="card-footer sideways-card" style="visibility:hidden;width:0px;height:0px;padding: 0 0 0 0;">ENEMY SCORE :<div style="visibility:hidden;width:0px;height:0px;padding: 0 0 0 0;" id="enemy-score"></div></div>
            <div class="card-footer sideways-card" style="visibility:hidden;width:0px;height:0px;padding: 0 0 0 0;">MY SCORE : <div style="visibility:hidden;width:0px;height:0px;padding: 0 0 0 0;" id="my-score"></div></div>
        </div>
        <script>
    ...
    Gui.addPage('Engine', `
    <div class="rendered-form" id="Engine-tab">
    <div class="card">
            <div class="card-body">
    ...
    Gui.addPage('Visual', `
    <div class="rendered-form" id="Visual-tab">
    <div class="card">
            <div class="card-body">
...
function openGUI() {
    ...
    Gui.open(() => {
        ...
        const cardBodyElem = Gui.document.querySelector("#chessboard");
        ...
        const getViewChessElem = Gui.document.querySelector('#viewchess-btn');
        ...
        const btnCloseUIElem = Gui.document.getElementById("button-close-gui");
        ...
        if (isNotCompatibleBrowser()) {
            ...
            Gui.document.querySelector('#content').style.maxHeight = "200px";
        ...
        getViewChessElem.onclick = () => {
            if (cardBodyElem.style.display === 'none') {
                cardBodyElem.style.display = 'block';
            } else {
                cardBodyElem.style.display = 'none';
            }
        }
        ...
        {
            removeSiteMoveMarkings();
            if(displayMovesOnSiteElem.checked)
            {
                displayMovesOnSiteElem.click();
            }
        }
